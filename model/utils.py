...
def euclidean_dist(x, y):
    ...
    dict = torch.pow(x-y, 2).sum(2)
    return dict


def plot_x_batch(adata):
    ...
    post_adata.obsm['original'] = post_adata.X
    post_adata.obs['batch'] = np.array([str(adata.obs['batch'][i][0])
...
def plot_x_class(adata):
    ...
    post_adata.obsm['original'] = post_adata.X
    post_adata.obs['cell_ontology_class_reannotated'] = np.array([str(adata.obs['cell_ontology_class_reannotated'][i][0])
                                        for i in range(post_adata.n_obs)])
    sc.pp.neighbors(post_adata, use_rep="original", n_neighbors=15)
    sc.tl.umap(post_adata)
    fig, ax = plt.subplots(figsize=(7, 6))
    sc.pl.umap(post_adata, color=["cell_ontology_class_reannotated"], ax=ax, show=True)
    plt.show()